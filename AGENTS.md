...
- Concurrency-safe file writes (`ssm_cli/fsio.py::atomic_write_text`): many `ssm` processes run at once (parallel `ssm run`) and hit the same cache/config/credential files. An atomic write is only safe if each writer uses its **own** temp file — deriving the temp name from the target (`path + ".tmp"`) makes concurrent writers collide, corrupt the temp, and crash the loser with `FileNotFoundError` at `os.chmod`/`os.replace`. Use `tempfile.mkstemp(dir=target_dir)` + `fsync` + `os.replace`. All CLI JSON writers go through this one helper; do not hand-roll temp+rename elsewhere.
- Treat local state as untrusted on read: `ssm_cli` cache/config readers swallow `OSError`/`ValueError` (a corrupt or truncated file becomes a cache-miss / empty-config, never a traceback). `ssm_cli/main.py::_handle_errors` has a last-resort `except Exception` so no CLI command ever surfaces a raw Python traceback — but it must re-raise `click.exceptions.Exit`/`Abort` first (e.g. `run` propagates the child process exit code through it).
- API error envelope is `{"message": ...}` everywhere. flask-restx aborts already use it; `Api/errors/errors.py` fallbacks were aligned to it (were `{"error": ...}`). Both CLI (`ssm_cli/api.py::_error_message`) and frontend (`errorToast.ts`) read `message` first, then `error`. `Api/core.py` sets `RESTX_ERROR_404_HELP = False` — otherwise flask-restx appends "did you mean `<route template>`" to every 404, leaking internal URL patterns and burying messages like "Project not found".
- Test hermeticity gotchas: (1) `ssm_cli.auth` talks to the OS keyring; the SecretService/D-Bus backend can hang the whole suite on a dev workstation, so `tests/conftest.py` force-disables keyring for the session. (2) Importing `Api.core`/`Api.api` eagerly builds `Connection()` which creates Mongo indexes, so there is no way to unit-test HTTP responses without a live MongoDB — verify API error responses via a scratch server or an in-process `app.test_client()` pointed at a throwaway `mongo` container, not the CI suite.
- Project archiving (feature): projects carry an `archived` boolean — one word so it is identical in Mongo (snake_case) and API JSON (camelCase), sidestepping the existing `created_at`/`createdAt` split. List filtering is server-side but lives ONLY in `Engines/projects.py::list()` (Python-side, after fetch): `archived=True` keeps `doc.get("archived") is True`, default keeps `is not True` so legacy/missing-field docs count as active (equivalent to a `{"archived": {"$ne": True}}` query). Do NOT push the filter into `list_docs`/`list_by_ids` — the RBAC actor-context build (`Engines/rbac.py`) needs them to return all projects, so filtering there would drop archived projects from authorization/visibility. `GET /api/projects?archived=<bool>` parses with `inputs.boolean` (never `type=bool`). Archive/unarchive reuses one PATCH path: `update(slug, name=None, archived=None)` is a partial update, so sending only `archived` toggles it.
