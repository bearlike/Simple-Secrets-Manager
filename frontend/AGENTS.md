...
- For revealable secret values in config tables, combine `table-fixed` with explicit responsive column widths and a capped (`max-h-*`) scrollable value viewport to prevent long unbroken text from shifting row/action layout.
- When both secrets pages need similar reveal UX, prefer sharing a `secrets/table` UI layer (table shell + reveal value cell + row actions) while keeping each page's query/filter logic local to avoid over-coupled data abstractions.
