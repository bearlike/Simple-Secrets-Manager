...
def test_list_configs_parses_configs(monkeypatch):
    client = ApiClient("http://localhost:8080", token="t")

    def fake_request(**kwargs):
        assert kwargs["url"].endswith("/api/projects/payments/configs")
        return _response(
            200,
            {
                "configs": [
    ...
    configs = client.list_configs("payments")
    assert [c["slug"] for c in configs] == ["base", "prod"]
    assert configs[1]["parentSlug"] == "base"
