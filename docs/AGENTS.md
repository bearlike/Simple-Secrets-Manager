...
| `ENV_REFERENCE.md` | Generated (`../scripts/generate_env_reference.py`, `make env-ref`) — every env var `ServerSettings`/`ReloadSettings` declare, with default and description. Do not hand-edit; the settings classes are the source of truth. |
...
- **`CONTRIBUTING.md` replaced the `DEVELOPER_GUIDE.md`/`DEVELOPMENT.md` pair as a single contributor entry point.** The old split asked a new contributor to read two files where the second mostly deferred to the first; `CONTRIBUTING.md` now carries everything (prerequisites through commit conventions) in one document, and `README.md` is the separate, GitHub-conventional documentation index. Don't recreate the two-file split — extend `CONTRIBUTING.md` in place.
